...
class BoardOutException(Exception):
    def __str__(self):
        return "Вы не можете выстрелить за границы полей!"


class ShipsNearbyException(Exception):
    pass


class ShotPointException(Exception):
    def __str__(self):
...
class Ship:
    ...
    def search_ship_by_cord(coord, ships, desc):
        ...
        for cord in ship_cords:
            x, y = cord[0] - 1, cord[1] - 1
            if desc[y][x] == "■":
                symbols.append(desc[y][x])
        length = len(ship_cords)
        health = len(symbols)
        (x1, y1), (x2, y2) = ship_cords[0], ship_cords[-1]
        if abs(x2 - x1) == 0:
            direction = "vertical"
        else:
            direction = "horizontal"
        ship = Ship(length, (x1, y1), direction)
    ...
    def add_ship(self, length, start_coord, direction):
        try:
            ...
            if "×" in check_arr or "■" in check_arr:
                raise ShipsNearbyException()
        except BoardOutException:
            return False
        except ShipsNearbyException:
            return False
        else:
            for cord in ship_cords:
                x, y = cord
    ...
    def shot(self, target_cords):
        try:
            ...
            if self.out(Dot(x, y)):
                ...
                raise ShotPointException()
        except ShotPointException as e:
...
class User(Player):
    def ask(self):
        while True:
            string = None
            while not string:
                string = input("Введите 2 целевых координаты: ")
...
class Game:
    ...
    def greet():
        print("Приветствуем в игре «Морской бой»!\n\n"
              "Как играть?\n"
              "Перед игроком предстают 2 поля. Одно поле ваше, другое соперника. В качестве соперника играет\n"
              "компьютер. Задача игрока - уничтожить все вражеские корабли. Всего кораблей 7: один трёхпалубный,\n"
    ...
    def random_board(desc):
        ...
        ships = []
        while True:
            direct = choice(["vertical", "horizontal"])
            if direct == "vertical":
            ...
            while reply:
                ...
                if reply == "Hit":
                    ...
                    if len(self.AI_desc.get_data()["unbroken_ships"]) == 0:
                        self.print_boards()
                        print("Вы победили! Поздравляем :D !")
                        return
                    print("Корабль подбит!")
                    print("Вы ходите еще раз!")
                    self.print_boards()
                elif not reply:
                    print(choice(["Мимо!", "Не попал!"]))
            reply = True
            while reply:
